...
export default async (req) => {
  ...
  try {
    ...
    const withNotes = enrichAnalysisWithUserNotes(composed, userNotes);
    const parsed = await enrichAnalysisWithAiNutritionFallback(withNotes, { geminiKey: apiKey });
    ...
    const parseFailed = e?.userMessage || /JSON|parse model JSON|after array element|truncated|Empty response/i.test(e?.message || '');
    return jsonResponse({
      error: parseFailed
        ? (e.userMessage || 'Could not finish that scan — try again, or type coffee in Describe.')
